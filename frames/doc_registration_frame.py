...
class DocRegistrationFrame:
    ...
    def register_event_action(self):
        ...
        error_msg = self.db_controller.check_email_exists(self.email_entry.get_text())
        if 0 != len(error_msg):
            messagebox.showerror(ERROR_BOX_TITLE, error_msg)
        else:
            # Step 5: All information is fine now register the doctor information in data base
            error_msg = self.db_controller.db_register_doctor_info(self.name_entry.get_text(),
                                                                   self.email_entry.get_text(),
                                                                   self.password_entry.get_text(),
                                                                   self.contact_number_entry.get_text(),
                                                                   self.mobile_number_entry.get_text(),
                                                                   self.clinic_name_entry.get_text(),
                                                                   self.clinic_address_entry.get_text())
