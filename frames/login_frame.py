...
class LoginFrame:
    ...
    def login_event_action(self):
        ...
        error_msg = self.db_controller.db_check_doctor_info(self.email_entry.get_text(),
                                                            self.password_entry.get_text())
        if 0 != len(error_msg):
            messagebox.showerror(ERROR_BOX_TITLE, error_msg)
        else:
            # Enter the next frame which is entry frame, which is the entry frame at index 1.
