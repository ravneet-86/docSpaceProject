...
class EntryFrame:
    ...
    def search_remedy_event_action(self):
        self.frame.enter_next_frame(4, True)
    ...
    def list_box_double_click_event_action(self, event):
        ...
        if 0 != len(self.curr_search_results_display):
            print("curSelection[0]", self.curr_search_results_display[cur_selection[0]])
    ...
    def __init__(self, root, ws, hs, db_controller):
        ...
        tk.Button(field_frame, text=SEARCH_REMEDY,
                  command=lambda: self.search_remedy_event_action(), font=WIDGET_FONT).grid(
            row=row_idx, column=1, pady=(0, 30))
